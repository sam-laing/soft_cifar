...
def make_datasets(reader: ReaderSoft, split_ratio:list = [0.8, 0.05, 0.15], use_hard_labels:bool = False, entropy_threshold:float = None, do_augment:bool = True):
...
def mixup_datapoints(x1,x2, y1,y2, p=0.5):
    '''
...
def mixup_loader(loader:DataLoader, device, alpha=0.2, ):
    '''
    ...
    for x1, y1 in loader:
        ...
        x, y = mixup_datapoints(x1, x2, y1, y2, p)
